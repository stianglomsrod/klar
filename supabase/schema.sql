...
CREATE POLICY "Admins can manage substitute_assignments"
  ON public.substitute_assignments FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'teacher' AND is_admin = true))
  WITH CHECK (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'teacher' AND is_admin = true));

CREATE POLICY "Substitutes can view own assignments"
  ON public.substitute_assignments FOR SELECT TO authenticated
...
AS $$ SELECT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'teacher' AND is_substitute = false); $$;
GRANT EXECUTE ON FUNCTION public.is_full_teacher() TO authenticated;

CREATE OR REPLACE FUNCTION public.is_substitute()
RETURNS boolean LANGUAGE sql SECURITY DEFINER SET search_path = '' STABLE
AS $$ SELECT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'teacher' AND is_substitute = true); $$;
GRANT EXECUTE ON FUNCTION public.is_substitute() TO authenticated;

CREATE OR REPLACE FUNCTION public.is_admin_teacher()
RETURNS boolean LANGUAGE sql SECURITY DEFINER SET search_path = '' STABLE
AS $$ SELECT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'teacher' AND is_admin = true); $$;
...
AS $$
  SELECT CASE
    WHEN NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'teacher') THEN false
    WHEN NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_substitute = true) THEN true
    ELSE EXISTS (
      SELECT 1 FROM public.substitute_assignments sa
      WHERE sa.substitute_id = auth.uid()
        AND (sa.student_id = p_student_id OR sa.class_id = (SELECT sp.class_id FROM public.student_profiles sp WHERE sp.id = p_student_id))
    )
  END;
$$;
...
AS $$
  SELECT CASE
    WHEN NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'teacher') THEN false
    WHEN NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_substitute = true) THEN true
    ELSE EXISTS (SELECT 1 FROM public.substitute_assignments sa WHERE sa.substitute_id = auth.uid() AND sa.class_id = p_class_id)
  END;
$$;
...
--
-- Pattern: Full teachers get unrestricted access (same as pre-RLS).
--          Students access own data (student_id/id = auth.uid()).
--          Substitutes get scoped SELECT via can_access_student()/can_access_class().
--          Substitute WRITES blocked on most tables (is_full_teacher() checks).
--
-- See migration 20260319100000_substitute_rls.sql for all 50+ policies.