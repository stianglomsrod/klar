...
| En manuell 18. juli-test rapporterte hydration mismatch på elevdagen: nettleseren hadde gjenopprettet `open` på et serverrendret `<details>` før React hydrerte. | Erstattet øktgruppenes native disclosure med en kontrollert knapp og panel. Første klient- og serverrender er alltid lukket; tilstanden uttrykkes med `aria-expanded`, `aria-controls` og `hidden`. | `student.spec` forsøker å mutere den gamle native disclosureen før hydration og kontrollerer lukket→åpen→lukket uten runtime-feil. `task-iteration.spec` åpner gruppen og bekrefter at en fersk autentisert elevside starter deterministisk lukket før den åpnes igjen uten runtime-feil. |
| Første fulle E3-WebKit-runde fikk 57/58 fordi en hard test-`reload()` avbrøt en legitim Next.js RSC-oppdatering med WebKits `Load failed`. | Beholdt alle runtime-feilkontroller, men lot persistensbeviset bruke en fersk autentisert elevside i stedet for å erstatte dokumentet midt i RSC-trafikken. | Målrettet WebKit-retest bestod 5/5 og bekreftet både varig oppgaveiterasjon og deterministisk lukket disclosure; endelig full WebKit-matrise bestod 58/58. |
| Axe målte 4,33:1 på den visuelt tonede etiketten «Forrige økt» under en målrettet hjelpekø-retest. | Økte bare sekundær tekstfarge fra `slate-600` til `slate-700`, uten å endre hierarki eller handlinger. | Målrettet hjelpekø-E2E og full ansattmatrise bestod uten axe A/AA-avvik. |
