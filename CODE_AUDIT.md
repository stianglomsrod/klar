...
**Resolution:** Extracted to `QuizBuilder.tsx` (190L), `RecipientPicker.tsx` (427L), `SchedulePicker.tsx` (357L). Monolith reduced from 1,537 → 667 lines (57% reduction). Subject creation duplication resolved via `resolveSubjectId()` helper.
...
|------|----------------------|
| `ClassesAccordion.tsx` (L221–247) | "Add class", "Edit trinn", "Add student", "Message class", "Edit class name", "Move student", "Remove student" |
| `StudentTable.tsx` (L68–81) | "View student profile", "Move student", "Remove student" |
...
2. ✅ Extract `src/components/teacher/ClassCombobox.tsx` (~170 lines)
...
**Scope:** ~870 lines removed from `CreateTaskModal.tsx` (1,537 → 667, 57% reduction)
**Risk:** Medium-High (complex form with many states)
**Goal:** ~~Break the 1,606-line modal into focused sub-components.~~ Done.

**Tasks:**
1. ✅ Extract `src/components/teacher/QuizBuilder.tsx` (190 lines)
   - Quiz question add/remove, option management. Controlled component via `questions`/`onQuestionsChange` props.
...
3. ✅ Extract `src/components/teacher/SchedulePicker.tsx` (357 lines)
   - Week navigation, schedule entry selection, Popover-based UI. `forwardRef` with `getSelectedEntryIds()` handle. Uses `fetchScheduleFallback`.
...
- `src/components/teacher/SchedulePicker.tsx` (357 lines)

**Files modified (1):**
- `src/components/teacher/CreateTaskModal.tsx` (1,537 → 667 lines)
...
| 2 | `teacher/CreateTaskModal.tsx` | ~~1,606~~ ~~1,530~~ 667 |
...
| 11 | `teacher/ClassesAccordion.tsx` | 598 |
